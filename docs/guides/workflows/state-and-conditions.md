...
| Chat attachments (test harness / API) | `attachments` | List of uploaded file metadata when present (`storage_key`, `mime_type`, `name`, `type`, `url`) |
...
Datetime context (seeded on every workflow run):

```
Now: {{__studio_now}}
Timezone: {{__studio_timezone}}
Locale: {{__studio_locale}}
```

`{{__studio_now}}` is recomputed at interpolation time (not frozen to the bootstrap snapshot). Standalone agent instructions support the same three placeholders (resolved from app timezone/locale).
